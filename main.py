...
SPORTMONKS_API_KEY = os.getenv("Sportmonks") TELEGRAM_TOKEN = os.getenv("Telegramtoken") TELEGRAM_CHAT_ID = os.getenv("Chatid")

bot = telegram.Bot(token=TELEGRAM_TOKEN) PARTIDOS_DEL_DIA = []

Zonas horarias

utc = pytz.utc madrid = pytz.timezone("Europe/Madrid")

Estados considerados como "en juego"

ESTADOS_EN_JUEGO = {"inplay_1st_half", "inplay_2nd_half", "et", "pen_live", "ht"}  # normalizado en minúsculas

Sesión con reintentos

session = requests.Session() retries = Retry(total=5, backoff_factor=2, status_forcelist=[429, 500, 502, 503, 504]) session.mount("https://", HTTPAdapter(max_retries=retries))

def enviar_mensaje(mensaje): try: print(f"[ENVÍO] Enviando mensaje Telegram:\n{mensaje}") bot.send_message(chat_id=TELEGRAM_CHAT_ID, text=mensaje, parse_mode=telegram.ParseMode.MARKDOWN) return True except Exception as e: print(f"[ERROR] No se pudo enviar mensaje: {e}") return False

def obtener_partidos(): global PARTIDOS_DEL_DIA hoy = datetime.datetime.utcnow().strftime("%Y-%m-%d") print(f"[INFO] Solicitando partidos del {hoy}...")
...
def obtener_fixture(fixture_id): url = f"https://api.sportmonks.com/v3/football/fixtures/{fixture_id}?api_token={SPORTMONKS_API_KEY}&include=events" try: response = session.get(url, timeout=10) return response.json().get("data", {}) except Exception as e: print(f"[ERROR] Falló la consulta del fixture {fixture_id}: {e}") return {}
...
while partidos_pendientes:
    ...
    for partido in partidos_activos:
        ...
        status = fixture.get("status", {}).get("state", "").lower()
...
def enviar_partidos(): mensaje = obtener_partidos() enviar_mensaje(mensaje)

if name == "main": enviar_partidos() monitorear_eventos()
