...
# --------------------------------------------
# Ejecución programada y control horario
# --------------------------------------------
def run_scheduler():
    ...
    while True:
        ...
        print(f"[{ahora.strftime('%H:%M:%S')}] Bot activo...")

        if ahora.hour == 9 and ahora.minute == 0 and not ejecutado_hoy_9:
            print("[INFO] Enviando resumen diario de las 09:00")
            daily_summary()
            ejecutado_hoy_9 = True

        if ahora.hour == 0 and ahora.minute == 1 and not ejecutado_hoy_0:
            print("[INFO] Enviando resumen diario de las 00:01")
            daily_summary()
            ejecutado_hoy_0 = True

        if ahora.hour == 12 and ahora.minute == 53:
            print("[INFO] Enviando resumen diario de las 12:41 (prueba)")
            daily_summary()
        ...
        if ahora.hour == 1 and ahora.minute == 0:
            ejecutado_hoy_9 = False
            ejecutado_hoy_0 = False
            print("[INFO] Flags reiniciadas para nuevo día")

        # Si estamos en horario de partidos, iniciar monitoreo
        if 12 <= ahora.hour or ahora.hour <= 4:
            print("[INFO] Horario de partidos detectado, iniciando monitoreo en vivo")
...
# --------------------------------------------
# Ejecutar scheduler al iniciar el script
# --------------------------------------------
